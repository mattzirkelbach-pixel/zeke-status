...
## DEFERRED-IS-NOT-FAILED (found & fixed 2026-09-06)
**Symptom:** `alerts/alert_state.json` cooldown timestamps for `premarket_thesis`, `premarket_go`, `major_move`, `gdx_add_zone` were frozen at 2026-04-18 -- the exact date of the "defer all ACTION alerts to morning briefing" policy in `alert_dispatcher.send_alert()`. Effect: `trade_alerts.py` re-generated and re-queued the same alerts every 10-min scan, 7 days/week (including weekends), producing ~30 `send_telegram_<hash>` rows/day in `state/deferred-alerts.jsonl` and zero rows in `state/alert-quality-log.jsonl` from `trade_alerts.py` since 4/18.
**Root cause:** `trade_alerts.py execute()` only called `_mark_sent()` when `send_telegram()` returned `True`. `send_telegram()` returns whatever `alert_dispatcher.send_alert()` returns, and `send_alert()` returns `False` both for "actually failed" and for "deferred to the morning briefing" (the market-hours ACTION-defer gate, ~line 517) -- a boolean return can't distinguish the two. Every ACTION alert has hit that deferred path since 4/18, so the cooldown clock never advanced and the alert never left the queue.
**Fix:** `alert_dispatcher.py` got a module-level `last_outcome` flag (`'sent'|'deferred'|'suppressed'|'failed'`) set at every return point in `send_alert()` -- `send_alert()`'s bool contract itself is unchanged (20+ callers depend on it). `trade_alerts.send_telegram()` now passes `urgency=alert['type']` through so deferred rows carry the real alert type instead of an opaque hash. `execute()` now calls `_mark_sent()` when the outcome is `'deferred'` too, not just on a real send. The `:520` action-defer branch and `trade_alerts._log_alert_quality` now both log `reason="action_deferred"` rows (previously unlogged entirely); `zeke-qc.py check_alert_suppression_rates`'s `is_digest_deferred` check was extended to bucket `action_deferred` alongside `digest_deferred` so this doesn't false-flag as `ALERT_NOISE_FLOOR`. Added a weekday guard to `check_premarket_thesis` (previously the only Tier-1 check missing one -- `check_morning_briefing` already had it).
**Rule:** any per-alert-type cooldown/dedup state that's advanced only on a literal `True` return from a dispatcher call is one dispatcher-side policy change away from freezing forever. If a dispatcher can legitimately return `False` for more than one reason ("dropped" vs "handled differently"), the caller needs a way to distinguish them -- a bool alone is not enough state to build a liveness gate on. Also see [[CLAUDE-CODE-EDIT-TOOL-SILENT-NOOP-UNDER-CHECK-RETIREMENTS-HOOK]] -- hit again during this fix (`Edit` no-op'd against `alert_dispatcher.py`'s self-registered capability match); worked around with the documented Bash+python3-heredoc fallback.

## LOG-ALERT-QUALITY-ARITY-REGRESSION (found & fixed 2026-09-07, broke since 2026-09-06)
**Symptom:** LaunchAgent `com.zeke.trade-alerts` exited 1 on every 10-min tick starting 2026-09-06. `/tmp/trade-alerts-err.log` showed `TypeError: _log_alert_quality() takes from 1 to 2 positional arguments but 3 were given` on every run; `/tmp/trade-alerts.log` looked deceptively healthy (each cycle logged its normal source-gate/DEFERRED lines right up until the crash).
**Root cause:** the same-day `DEFERRED-IS-NOT-FAILED` fix (above) added a third call-site argument, `_log_alert_quality(sent_alerts, suppressed_alerts, deferred_alerts)`, and a `for alert in (deferred or []):` loop body reading a free variable `deferred` -- but never added `deferred` as a third parameter to the function signature (`def _log_alert_quality(alerts_sent, suppressed=None)`). Every run crashed inside `execute()` after successfully sending/deferring alerts, so `_save_alert_state()` never ran either -- state was silently unpersisted for ~24h alongside the exit-1.
**Fix:** added `deferred: list[dict] | None = None` as the third parameter (the loop body already referenced the right name, it just wasn't bound). Verified via direct run (`exit 0`, `Logged N quality entries`) then `launchctl unload/load` to clear the stale exit-1 latched in `launchctl list`.
**Rule:** when a fix adds a new call-site argument to an existing function, grep the function's `def` line in the same edit -- a stale default-arg signature will pass most eyeball review (the diff "looks right" at the call site) and silently pass py_compile (arity mismatches are a runtime TypeError, not a syntax error). QC/launchd exit-status monitoring caught this in <24h; a `_save_alert_state()` call succeeding *before* the crashing line would have masked it further by letting cooldown state advance even on the broken path.
...
## ZOMBIE-CLAUDE-KILL run 2026-09-18 04:39 UTC
- pid=15060 ppid=1 etime=11219s cpu=0.1% state=R match=gui reason=etime=11219s state=R
Killer: scripts/zombie-claude-killer.py (DO-NOT-REBUILD-zombie-claude-killer).

## ZOMBIE-CLAUDE-KILL run 2026-09-19 03:10 UTC
- pid=55534 ppid=1 etime=56666s cpu=0.1% state=R match=gui reason=etime=56666s state=R
Killer: scripts/zombie-claude-killer.py (DO-NOT-REBUILD-zombie-claude-killer).

## ZOMBIE-KILLER-STATE-R-FALSE-POSITIVE (fixed 2026-09-21)
`scripts/zombie-claude-killer.py` had a second kill rule, `etime>2h AND ps state startswith R`, with no CPU guard. The Claude.app GUI main process idles in state R (Electron run loop), so that rule SIGKILLed a healthy app on every one of its 16 firings between 2026-05-07 and 2026-09-19 (max 3.8% CPU, most 0.0-1.0%). Each kill silently took the in-app Cowork scheduler down with it, and Cowork does NOT catch up a missed fire after relaunch — that is the root cause of the `morning_alpha_synthesis` absence incidents (abs-20260917T140500Z and earlier). The branch is deleted; only `cpu>50` remains, which still covers the 2026-05-03 64h/100%-CPU incident that justified the script. Do NOT re-add a state-based rule. Diagnostic order when a Cowork surface is missing: `state/incidents/zombie-kills.jsonl` → `ps` for Claude.app → `~/Library/Logs/Claude/main.log` "Confirmed task run for:".

## DEFERRED-QUEUE-BYPASSED-COOLDOWN (fixed 2026-09-21)
**Symptom:** `options_risk_critical` (cooldown_key `options_risk:critical:daily`) landed in `state/deferred-alerts.jsonl` twice/day (13:45Z + 20:15Z) 7d/week; `multi_scale_top_fire` queued 4 identical rows on 4 consecutive days (week of 2026-09-14: 14/42 deferred rows were dupes).
